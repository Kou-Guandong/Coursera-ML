...
numTrials = length(trialValues);

errorRow = 0;

results = eye(numTrials^2, 3);

for i=1:numTrials
    for j=1:numTrials
        C_trial = trialValues(i);
        sigma_trial = trialValues(j);
        ...
        errorRow = errorRow + 1;
        results(errorRow, :) = [C_trial, sigma_trial, prediction_error];
    end
end
